...
def _rowOffsetColGain_zeroImag(d, sz):
    for i in range(sz):
        d[float(i)] = dis.nw.matrix([[float(i), 2*float(i)], 
                                     [10+float(i), 10+2*float(i)]])
    return d

def _rowOffsetColGain_posImag(d, sz):
    for i in range(sz):
        d[float(i)+1j] = dis.nw.matrix([[float(i), 2*float(i)], 
                                        [10+float(i), 10+2*float(i)]])
    return d
...
def _rowOffsetColGain_posNegImag(d, rg):
    for i in range(-rg,rg+1):
        d[float(i)-float(i)*1j] = dis.nw.matrix([[float(i), 2*float(i)], 
                                                [10+float(i), 10+2*float(i)]])
    return d

def _getUniMat():
    return dis.nw.matrix([[pow(2.,-.5), pow(2.,-.5),0], 
                          [-pow(2.,-.5)*1.j, pow(2.,-.5)*1.j,0],
                          [0, 0,1.j]])

def _getNonUniMat():
    return dis.nw.matrix([[pow(3.,-.5), pow(2.,-.5),0], 
...
def getIdentity():
    return dis.nw.matrix([[1., 1.], 
                          [1., 1.]])

def _getComplex1():
    return dis.nw.matrix([[1.+1.j, 2.+2.j], 
                          [3.+3.j, 4.+4.j]])

def getComplex1Absolute():
    return dis.nw.matrix([[1.4142135623730950488016887242097, 2.8284271247461900976033774484194], 
